...
async function aiExtract(text,name){
  try{const r=await fetch('/api/extract',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text,projectName:name})});const d=await r.json();return d.phases||null}catch(e){return null}
}
...
.oc{background:var(--sf);border:1px solid var(--bd);border-radius:12px;padding:28px;max-width:620px;width:100%;position:relative;max-height:90vh;overflow-y:auto}
...
.atp{background:var(--sf3);border:1px solid var(--bd);border-radius:8px;padding:10px 12px;margin-bottom:6px;display:flex;align-items:flex-start;gap:10px}
...
.phase-hdr{font-family:var(--fd);font-size:11px;letter-spacing:2px;text-transform:uppercase;color:var(--am);padding:12px 0 8px;border-bottom:1px solid var(--bd);margin:16px 0 10px;display:flex;align-items:center;justify-content:space-between}
.phase-hdr:first-child{margin-top:0}
.phase-hdr .phase-count{font-family:var(--fm);font-size:10px;color:var(--txd);font-weight:400}
.pct-ring{position:relative;width:56px;height:56px;flex-shrink:0}
.pct-ring svg{transform:rotate(-90deg)}
.pct-ring .pct-text{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:var(--fd);font-size:13px;font-weight:700}
`
...
export default function Cockpit({data,db,reload}){
  ...
  // Progress helpers
  const pctDone=(id)=>{const pt=tasks.filter(t=>t.projectId===id);if(!pt.length)return 0;return Math.round(pt.filter(t=>t.status==='done').length/pt.length*100)}
  const phasePct=(id,cat)=>{const pt=tasks.filter(t=>t.projectId===id&&t.category===cat);if(!pt.length)return 0;return Math.round(pt.filter(t=>t.status==='done').length/pt.length*100)}

  const R=async()=>await reload()
  ...
  // AI import — now handles phases
  const doAi=async()=>{if(!aiT.trim())return;sAiL(true);sAiE('');const r=await aiExtract(aiT,curP?.name||'');sAiL(false);if(!r||!Array.isArray(r)||!r.length){sAiE('Não encontrei etapas/tarefas acionáveis nesse texto.');return}sAiP(r.map(ph=>({...ph,sel:true,tasks:ph.tasks.map(t=>({...t,sel:true}))})))}
  const togAiPhase=(i)=>sAiP(p=>p.map((ph,j)=>j===i?{...ph,sel:!ph.sel,tasks:ph.tasks.map(t=>({...t,sel:!ph.sel}))}:ph))
  const togAiTask=(pi,ti)=>sAiP(p=>p.map((ph,j)=>j===pi?{...ph,tasks:ph.tasks.map((t,k)=>k===ti?{...t,sel:!t.sel}:t)}:ph))
  const confirmAi=async()=>{
    const mx=Math.max(0,...tasks.filter(t=>t.projectId===pid).map(t=>t.order||0))
    const newTasks=[];let ord=mx+1
    aiP.forEach(ph=>{if(!ph.sel)return;ph.tasks.forEach(t=>{if(!t.sel)return;newTasks.push({projectId:pid,title:t.title,description:t.description||'',priority:t.priority||'medium',category:ph.phase,order:ord++})})})
    if(newTasks.length)await db.insertTasksBatch(newTasks)
    sAiP(null);sAiT('');setOv(null);await R()
  }
  ...
  // % ring component
  const PctRing=({pct,color,size=56})=>{const r=22,c=2*Math.PI*r;return(
    <div className="pct-ring" style={{width:size,height:size}}>
      <svg width={size} height={size} viewBox="0 0 56 56"><circle cx="28" cy="28" r={r} fill="none" stroke="var(--bd)" strokeWidth="4"/><circle cx="28" cy="28" r={r} fill="none" stroke={color||'var(--gn)'} strokeWidth="4" strokeDasharray={c} strokeDashoffset={c-(pct/100)*c} strokeLinecap="round"/></svg>
      <div className="pct-text" style={{color:color||'var(--gn)'}}>{pct}%</div>
    </div>
  )}
  ...
  const Prog=({id,color})=>{const pt=tasks.filter(t=>t.projectId===id);if(!pt.length)return null;const d=pt.filter(t=>t.status==='done').length;const pct=Math.round(d/pt.length*100);return(<div className="tp"><div className="tpb"><div className="tpf" style={{width:`${pct}%`,background:`linear-gradient(90deg,var(--gn),${color||'var(--cy)'})`}}/></div><span className="tpl">{d}/{pt.length} ({pct}%)</span></div>)}
  ...
  // Group tasks by category for phase view
  const grouped=cats.map(c=>({name:c,tasks:ft2.filter(t=>t.category===c),pct:phasePct(proj.id,c)}))
  const uncategorized=ft2.filter(t=>!t.category)

  return(<><style>{CSS}</style><div className="ck"><div className="in">
    <button className="bk" onClick={()=>{setView('dash');setPid(null)}}>← Dashboard</button>
    <div className="fb" style={{marginBottom:24}}>
      <div style={{display:'flex',alignItems:'center',gap:20}}>
        <PctRing pct={pctDone(proj.id)} color={proj.color} size={64}/>
        <div><h2 style={{fontFamily:'var(--fd)',fontSize:24,color:proj.color,letterSpacing:2}}>{proj.name}</h2><p style={{fontSize:13,color:'var(--txd)',marginTop:4}}>{proj.description}</p>{proj.star&&<div style={{marginTop:10,fontFamily:'var(--fm)',fontSize:11,padding:'8px 12px',background:'var(--sf)',borderRadius:6,borderLeft:`2px solid ${proj.color}`}}>★ {proj.star}</div>}</div>
      </div>
      <div className="g8"><button className="b bgr" onClick={()=>startSess(proj.id)}>▶ Iniciar Sessão</button>{proj.status==='active'&&<button className="b bs bg" onClick={()=>parkProj(proj.id)}>Estacionar</button>}</div>
    </div>
    ...
    {ptab==='tasks'&&<div>
      ...
      <Prog id={proj.id} color={proj.color}/>
      ...
      {cats.length>0&&<><span style={{fontFamily:'var(--fm)',fontSize:9,color:'var(--txd)',letterSpacing:1,marginLeft:8}}>ETAPA:</span><button className={`fc ${tcf==='all'?'on':''}`} onClick={()=>setTcf('all')}>Todas</button>{cats.map(c=><button key={c} className={`fc ${tcf===c?'on':''}`} onClick={()=>setTcf(c)}>{c}</button>)}</>}</div>

      {ft2.length===0?<div className="emp">📋 Nenhuma tarefa {tf!=='all'?'com esse filtro':'ainda'}.</div>:<>
        {/* Grouped by phase */}
        {tcf==='all'&&cats.length>0?<>
          {grouped.map(g=>g.tasks.length>0&&<div key={g.name}>
            <div className="phase-hdr"><span>{g.name}</span><span className="phase-count">{g.tasks.filter(t=>t.status==='done').length}/{g.tasks.length} · {g.pct}%</span></div>
            {g.tasks.map(t=><TI key={t.id} t={t}/>)}
          </div>)}
          {uncategorized.length>0&&<><div className="phase-hdr"><span>Sem etapa</span></div>{uncategorized.map(t=><TI key={t.id} t={t}/>)}</>}
        </>:ft2.map(t=><TI key={t.id} t={t}/>)}
      </>}
    </div>}
  ...
  {ov==='task'&&<div className="ov"><div className="oc"><h3 style={{fontFamily:'var(--fd)',fontSize:14,letterSpacing:2,color:'var(--cy)',marginBottom:20}}>NOVA TAREFA</h3>
    ...
    <div className="fg"><label className="fl">Etapa / Fase</label><input className="fi" value={fTC} onChange={e=>sFTC(e.target.value)} placeholder="Ex: Sprint 1, Fundação, MVP..."/></div>
  ...
  {ov==='ai'&&<div className="ov"><div className="oc"><h3 style={{fontFamily:'var(--fd)',fontSize:14,letterSpacing:2,color:'var(--am)',marginBottom:6}}>⚡ IMPORTAR COM IA</h3>
    <p style={{fontSize:12,color:'var(--txd)',marginBottom:20,lineHeight:1.6}}>Cole a documentação de um projeto. A IA identifica as etapas e extrai as tarefas de cada uma pra você revisar.</p>
    {!aiP?<>
    <div className="fg"><label className="fl">Texto do documento</label><textarea className="ft" value={aiT} onChange={e=>sAiT(e.target.value)} placeholder="Cole aqui a doc do projeto..." style={{minHeight:180,fontFamily:'var(--fm)',fontSize:12}}/></div>
    ...
    {aiL?<div className="ail"><div className="sp"/>Analisando documento e mapeando etapas...</div>:<div className="g8"><button className="b ba" onClick={doAi} disabled={!aiT.trim()}>Mapear projeto</button><button className="b bg" onClick={()=>setOv(null)}>Cancelar</button></div>}
    </>:<>
    <div style={{fontFamily:'var(--fm)',fontSize:10,color:'var(--gn)',letterSpacing:1,marginBottom:16}}>✓ {aiP.length} etapas · {aiP.reduce((a,p)=>a+p.tasks.length,0)} tarefas encontradas</div>

    {aiP.map((ph,pi)=>{const selCount=ph.tasks.filter(t=>t.sel).length;return(<div key={pi} style={{marginBottom:16}}>
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:8,cursor:'pointer'}} onClick={()=>togAiPhase(pi)}>
        <div style={{width:20,height:20,borderRadius:4,border:`1.5px solid ${ph.sel?'var(--am)':'var(--txd)'}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,color:ph.sel?'var(--am)':'transparent',background:ph.sel?'var(--amg)':'transparent'}}>{ph.sel?'✓':''}</div>
        <span style={{fontFamily:'var(--fd)',fontSize:11,letterSpacing:2,color:'var(--am)',textTransform:'uppercase'}}>{ph.phase}</span>
        <span style={{fontFamily:'var(--fm)',fontSize:10,color:'var(--txd)'}}>{selCount}/{ph.tasks.length} tarefas</span>
      </div>
      {ph.tasks.map((t,ti)=>{const pr=PRI[t.priority]||PRI.medium;return(
        <div key={ti} className="atp" style={{opacity:t.sel?1:.35,marginLeft:12}}>
          <div style={{width:18,height:18,borderRadius:4,border:`1.5px solid ${t.sel?'var(--cy)':'var(--txd)'}`,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:10,color:t.sel?'var(--cy)':'transparent',background:t.sel?'var(--cyg)':'transparent',flexShrink:0}} onClick={()=>togAiTask(pi,ti)}>{t.sel?'✓':''}</div>
          <div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:'var(--txb)'}}>{t.title}</div>{t.description&&<div style={{fontSize:11,color:'var(--txd)',marginTop:2}}>{t.description}</div>}
          <div style={{fontFamily:'var(--fm)',fontSize:9,color:'var(--txd)',marginTop:4,display:'flex',gap:8}}><span style={{color:pr.color}}>{pr.label}</span>{t.time_estimate&&<span>~{t.time_estimate}</span>}</div></div>
        </div>
      )})}
    </div>)})}

    <div className="g8" style={{marginTop:16}}>
      <button className="b bgr" onClick={confirmAi}>Importar {aiP.reduce((a,p)=>a+p.tasks.filter(t=>t.sel).length,0)} tarefas</button>
      <button className="b bg" onClick={()=>sAiP(null)}>Voltar</button>
      <button className="b bg" onClick={()=>{sAiP(null);sAiT('');setOv(null)}}>Cancelar</button>
    </div>
    </>}
  </div></div>}
  ...
  return(<><style>{CSS}</style><div className="ck"><div className="in">
    ...
    <div className="pg">
      {actP.map(proj=>{const ls3=lSess(proj.id);const pt2=tasks.filter(t=>t.projectId===proj.id);const dn=pt2.filter(t=>t.status==='done').length;const pn=pt2.filter(t=>t.status!=='done').length;const pct=pctDone(proj.id);return(
        <div key={proj.id} className="pc" onClick={()=>{setPid(proj.id);setView('proj');setPtab('tasks');setTf('all');setTcf('all')}}>
          ...
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:12}}>
            <div><div style={{fontFamily:'var(--fm)',fontSize:9,letterSpacing:2,color:proj.color}}>P{proj.priority}</div><h3 style={{color:proj.color}}>{proj.name}</h3></div>
            {pt2.length>0&&<PctRing pct={pct} color={proj.color}/>}
          </div>
          <div className="ds">{proj.description}</div>
          {proj.star&&<div className="st" style={{borderLeftColor:proj.color,color:'var(--txd)'}}>★ {proj.star}</div>}
          {pt2.length>0&&<div style={{marginBottom:12}}><div style={{display:'flex',justifyContent:'space-between',fontFamily:'var(--fm)',fontSize:10,color:'var(--txd)',marginBottom:6}}><span>{dn}/{pt2.length} tarefas</span><span>{pn} pendentes</span></div><div style={{height:3,background:'var(--bd)',borderRadius:99,overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:`linear-gradient(90deg,var(--gn),${proj.color})`,borderRadius:99,transition:'width .5s'}}/></div></div>}
    ...
    {parkP.length>0&&<div style={{marginTop:24}}><span className="hft" style={{display:'block',marginBottom:12}}>Estacionados</span>{parkP.map(p=>{const pct2=pctDone(p.id);return(<div key={p.id} className="pk"><div style={{display:'flex',alignItems:'center',gap:12}}>{tasks.filter(t=>t.projectId===p.id).length>0&&<PctRing pct={pct2} color={p.color} size={40}/>}<div><h4 style={{color:p.color}}>{p.name}</h4><span style={{fontSize:11,color:'var(--txd)'}}>{p.description}</span></div></div>{actP.length<2&&<button className="b bs bg" onClick={()=>actProj(p.id)}>Ativar</button>}</div>)})}</div>}
  </div></div>
  ...
  {ov==='proj'&&<div className="ov"><div className="oc"><h3 style={{fontFamily:'var(--fd)',fontSize:14,letterSpacing:2,color:'var(--cy)',marginBottom:20}}>NOVO PROJETO</h3>
    <div className="fg"><label className="fl">Nome</label><input className="fi" value={fPN} onChange={e=>sFPN(e.target.value)} placeholder="Ex: Content Engine"/></div>
